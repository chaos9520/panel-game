...
function Server:adjust_ratings(room, winning_player_number, gameID)
  ...
  if continue then
    ...
    for player_number = 1, 2 do
      ...
      if opponent == 1 and players[player_number].level > (math.min(8, math.max(1, math.floor(leaderboard.players[players[player_number].user_id].rating / 200) - 1))) and math.abs(players[1].level - players[2].level) <= 2 then
