...
There are quite a number of changes in the modern levels
...
#### Changes exclusive to Modern levels 1 to 11
...
I have recently ported over the Classic style levels over so that they are playable in Versus. There is no overtime mechanic for these levels.
...
- A modified version of exploding lift has been implemented. Stack raise cannot be stopped, and the stack cannot be raised manually while a clear is taking place. Also, the game ends when the stack hits the top, even while panels are active.
## Other Changes
